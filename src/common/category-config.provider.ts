...
export class CategoryConfigProvider implements OnModuleInit {
  ...
  async initialize() {
    try {
      ...
      const configs =
        (await this.categoryConfigService.findAll()) as unknown as CategoryConfigType[];
      ...
      if (configs.length > 0) {
        ...
        this.defaultCategory = defaultConfig
          ? defaultConfig.name
          : configs[0].name;
      ...
      console.error(
        'Error loading category configurations from database:',
        error,
      );
      // Fall back to static configuration
      this.useStaticConfig();
  ...
  async getCategoryConfig(category: string): Promise<CategoryConfigType> {
    ...
    const config = this.categoryConfigs.find(
      (config) => config.name.toLowerCase() === category.toLowerCase(),
    );
    return config;
  }
  ...
  private isApiAllowed(apiList: any[], path: string, method: string): boolean {
    return apiList.some((api) => {
      ...
      const methodMatches =
        api.methods.includes('*') || api.methods.includes(method.toUpperCase());
