...
describe('CategoryConfigService', () => {
  ...
  describe('findOne', () => {
    it('should return a single category config', async () => {
      ...
      expect(result).toEqual(mockCategoryConfig);
      expect(prisma.categoryConfig.findUnique).toHaveBeenCalledWith({ where: { id: 1 } });
    });
  ...
  describe('findByName', () => {
    it('should return a single category config by name', async () => {
      ...
      expect(result).toEqual(mockCategoryConfig);
      expect(prisma.categoryConfig.findUnique).toHaveBeenCalledWith({ where: { name: 'Test Category' } });
    });
  ...
  describe('findDefault', () => {
    it('should return the default category config', async () => {
        ...
        expect(prisma.categoryConfig.findFirst).toHaveBeenCalledWith({ where: { isDefault: true } });
      });
  ...
  describe('create', () => {
    ...
    it('should create a new category config', async () => {
      ...
      expect(prisma.categoryConfig.create).toHaveBeenCalledWith({ data: expect.any(Object) });
    });
  ...
  describe('update', () => {
    const updateDto: UpdateCategoryConfigDto = { name: 'Updated Category', swagger: { visible: false } };
    it('should update a category config', async () => {
        ...
        expect(prisma.categoryConfig.update).toHaveBeenCalledWith({ where: { id: 1 }, data: expect.any(Object) });
      });
  ...
  describe('remove', () => {
    it('should remove a category config', async () => {
        ...
        expect(result).toEqual(mockCategoryConfig);
        expect(prisma.categoryConfig.delete).toHaveBeenCalledWith({ where: { id: 1 } });
      });
